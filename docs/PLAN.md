...
|---|---|---|---|---|---|---|
| AK-001 runner-cli-contract-spike | runner | S0 D1-D3: Claude CLI / Codex CLI (`codex exec`) / gh merge の公式 docs 確認、one-shot 実機 smoke、`docs/spike-results.md`、prompt_contract parse/schema fail-closed fixture、full matrix 実行計画。Codex SDK / Claude Agent SDK は paid-risk-gated deferred として別 gate に分離 | — | SPEC §9.1.1 evidence gate、§9.3 prompt_contract schema、§13.5 runner contract。N=20 統計閾値は分離後の runner adoption gate (#23 rewrite/replacement) で扱う | `e2e/runners/spike-runner-stability.ts`、prompt_contract fixture (`need_input` default あり/なし、paused schema、schema mismatch)、公式 docs evidence、one-shot smoke evidence、Codex CLI pinned evidence plan | runner 実装、同梱 asset 実装、full live matrix 実行、SDK / API key backed matrix 実行 |
...
| AK-009 claude-runner | runner | `claude-runner` auth/runner/resume/safety、Claude 4 phase read-only boundary | AK-001, Claude CLI adoption evidence gate (A gate only; Codex exec pinned evidence is not a blocker), AK-002, AK-004, AK-005, migration parent #31 complete | §9.1.1 A、§11.4.3、§13.4 Claude safety | `claude-runner/safety`、mock runner、manual hello world、Claude CLI adoption evidence | Codex runner、Codex exec evidence gate、workflow orchestration |
| AK-010 codex-cli-exec-runner | runner | `codex-runner` auth/runner/resume/sandbox、`codex exec` subprocess, JSONL parser, final JSON / output schema validation, ChatGPT-managed auth probe, API key rejection, process group, hard timeout, `need_input` final-output turn loop | AK-001, Codex exec evidence gate (#23 rewrite/replacement after #36 + MIG-004 #35), AK-002, AK-004, AK-005, migration parent #31 complete | §9.1.1 B、§13.1 question、§13.4 sandbox/timeout、Codex auth file / `CODEX_API_KEY` safety | `codex-runner` CLI subprocess mock、JSONL event fixture、final JSON schema fixture、resume fixture、API key rejection fixture、auth file redaction fixture、manual hello world after #35 evidence | Claude runner、workflow orchestration、Codex SDK runner |
...
- [ ] full matrix は follow-up #23 の rewrite/replacement に分離し、AK-009 は Claude CLI gate、AK-010 は `codex exec` evidence gate として扱う。現行 #23 本文のまま実行しない

**Runner adoption gate (follow-up #23 rewrite/replacement、AK-009 / AK-010 の採用済み扱い前に必須):**
...
- v0.1.0 blocker ではない。full matrix は paid-risk-gated deferred とし、subscription / billing 扱い確認または operator 明示承認なしに実行しない。
- [ ] N=50 試行で構造化出力成功率 **>= 95%**
...
- [ ] D3: `codex-runner/resume.ts` (`codex exec` CLI resume + 失敗時 cold restart) + `codex-runner/sandbox.ts` (`permissions.codex.sandbox_mode` / `approval_policy` / `allow_network` / `home_isolation` の適用と検証)
  - `codex exec resume <session_id>` 形式、`resume --last` の扱い、TUI 回答の渡し方は MIG-004 pinned evidence に従う
