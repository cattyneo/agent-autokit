...
- 認証: claude / codex CLI の subscription 流用 (API key 不可)
- runner: `claude -p` primary + Codex SDK
...
- スケジュール
- public npm publish
- Claude Agent SDK primary 昇格 (S0 spike 結果次第)
...
|---|---|---|---|
| **S0: Runner / SDK Contract Spike** | **3** | — | **runner契約確定 + 主要API疎通** |
...
│   │   │   └── sdk-experimental.ts     # Agent SDK (S0 で動作確認できた場合)
│   │   └── package.json
│   ├── codex-runner/
│   │   ├── src/
│   │   │   ├── runner.ts               # runStreamed
│   │   │   ├── auth.ts
│   │   │   ├── resume.ts               # resumeThread
│   │   │   └── sandbox.ts              # sandbox / approval policy
│   │   └── package.json
│   └── tui/
│       ├── src/
│       │   ├── App.tsx
...
### S0: Runner / SDK Contract Spike (3日)

**目標:** 主要 runner / SDK の挙動を実機確認し、不確実性を S1 着手前に潰す。

- [ ] D1: Context7 / 公式 docs で最新仕様確認
  - Claude `claude -p` (CLI) の non-interactive オプション / 構造化出力 / resume 経路
  - Claude Agent SDK (TS) の auth / settingSources / 構造化出力 / resume
  - Codex SDK (TS) `runStreamed` / `resumeThread` / sandbox / approval policy
  - `gh pr merge --match-head-commit` / `--delete-branch` 動作
  - 各 runner / SDK の exact version、公式 docs URL + 確認日、実際に使う config key / option 名、unsupported API の有無を `docs/spike-results.md` に記録 (SPEC §9.1.1)
- [ ] D1-D2: spike script で疎通検証
  ...
  - Codex `runStreamed` で worktree 内編集 + テスト + 構造化出力
...
**B. primary Codex SDK (Codex phase 3 種: plan_verify / implement / fix、必達、未達で v0.1.0 出荷不可):**
- [ ] N=20 試行で `plan_verify` / `implement` / `fix` の prompt_contract YAML parse + §9.3 schema validation 成功率 **>= 95%**
- [ ] `need_input` + `question.default` あり / `question.default` なし / `paused` schema / schema mismatch の fixture が期待どおり pass/fail-closed する
- [ ] `resumeThread` resume 成功率 **100%** (3 phase × 1 試行 + 予備 2 = 5 試行で全成功)
- [ ] `runStreamed` sandbox 動作: `implement` / `fix` は `workspace-write` + `allow_network=false`、`plan_verify` は read-only (書込検出で `sandbox_violation`)
- [ ] S0 fixture の `.codex/skills/` または `.agents/skills/` 配下が prompt 内 skill 明示で参照可能
- [ ] subscription 認証 (`codex login`) で `OPENAI_API_KEY` unset でも動作
- [ ] `runStreamed` / `resumeThread` / config override / sandbox / approval policy の exact TypeScript API と config key が pinned version の型・公式 docs・実機で一致。未保証 API 依存なら S0 未達
...
- [ ] D2a: `packages/core` 作成 + env/auth 境界 (`AK-004`)
  ...
  - `env-allowlist.ts` (SPEC §9.5.1 / §11.1、env 境界の唯一の SoT):
    ...
    - 任意ユーザー env / `.env` 由来 env / `AUTOKIT_*` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` を両系統で遮断
...
**目標:** claude-runner / codex-runner 確立。
...
- [ ] D2: `claude-runner/sdk-experimental.ts` (S0 spike 採用基準 (SPEC §9.1.1) を満たした場合のみ実装)
- [ ] D3: `codex-runner/auth.ts` + `codex-runner/runner.ts` (runStreamed + 同等の status 転写)
- [ ] D3: `codex-runner/resume.ts` (resumeThread + 失敗時 cold restart) + `codex-runner/sandbox.ts` (`permissions.codex.sandbox_mode` / `approval_policy` / `allow_network` / `home_isolation` の適用と検証)
...
**Exit:** Claude (`claude -p`) / Codex 両方で:
...
- env から `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` が削除されて子プロセスに渡される
...
- [ ] D2: `workflows/implement.ts` — **7 checkpoint / 8 step atomic 永続化 (SPEC §7.3 / §4.2 / AC §13.1)**
  ...
  2. Codex runner で `implement` prompt_contract 起動 (内包 skill: `autokit-implement` + `autokit-question`、`permissions.codex.sandbox_mode=workspace-write` / `approval_policy=on-request` / `allow_network=false` / `home_isolation` 適用、test framework 取得必要時 `paused`+`network_required`)
...
- [ ] D4: `workflows/fix.ts` — **7 checkpoint atomic 永続化 (SPEC §7.5.2 / §4.2 / AC §13.1)、checkpoint 順序: `before_sha → rebase_done → agent_done → commit_done → push_done → head_sha_persisted → after_sha`**
  ...
  3. Codex runner で `fix` prompt_contract 起動 (内包 skill: `autokit-implement` + `autokit-question`、入力に `fix.origin="review"` なら sanitize 済 finding 群、`fix.origin="ci"` なら sanitize 済 CI failure log、§4.6.2.1)
...
| `core/env-allowlist` | 100% | `buildGhEnv()` (PATH/HOME/LANG/LC_*/TERM/TZ/GH_TOKEN/GITHUB_TOKEN/XDG_*) と `buildRunnerEnv()` (GitHub token 除外) の 2 系統分離、`ANTHROPIC_API_KEY`/`OPENAI_API_KEY`/任意ユーザー env/`.env` 由来/`AUTOKIT_*` は両系統で遮断、孫プロセスでも `process.env` 直接継承なし、ESLint custom rule で spread 禁止 |
| `core/model-resolver` | 90%+ | queued→planning 一括解決、resume 時再利用 |
| `core/doctor` | 80%+ | env unset FAIL、NFS WARN、prompt_contract 1:1、stale worktree、`permissions.codex.allow_network=true` + `home_isolation=shared` FAIL、release verification env preflight |
| `core/logger` | 85%+ | size cap ローテ atomic 手順 (flush→fsync→close→rename→open、event drop なし、rename 失敗時 旧 fd 継続+WARN 行記録、起動時 sweep 同手順)、sanitize→truncate 順序、`.env` 値 file:line 限定、`AUTOKIT_LOG=debug` 検出で doctor WARN、**audit kind table-driven テスト: SPEC §10.2.2.2 failure 系全 kind が `failure.code` 列挙 (§4.2.1.1) と 1:1 対応 (差分検出で test fail) + 各 paused/failed 遷移で event 本体に `failure: {phase, code, message, ts}` 4 field 含む**、**操作系 audit kind 14 種 (`resume`/`resumed`/`lock_seized`/`init_rollback`/`init_rollback_failed`/`retry_resumed`/`runner_idle`/`audit_hmac_key_rotated`/`queue_corruption_recovered`/`sanitize_pass_hmac`/`auto_merge_disabled`/`auto_merge_reserved`/`branch_deleted`/`retry_pr_closed`) の発火タイミング fixture テスト** (SPEC §10.2.2.1)、**audit event order 保証: tasks.yaml atomic write commit と audit event 書込を 1 critical section (rotate 判定はその外側) で行い、`state=paused` と対応 failure event の log file 跨ぎ分割を発生させない** |
...
| `claude-runner` / `codex-runner` | mock based 80%+ | status 値の許容セット (`completed` / `need_input` / `paused` / `failed` のみ受領、それ以外で FAIL)、§9.3 の 7 contract `data` schema validation、未知 field / enum 違反 / サイズ超過で `prompt_contract_violation`、`rate_limited` runner 層生成、`default` なし `status=need_input` で FAIL、process group SIGINT |
...
- `claude-runner` / `codex-runner`: 実 runner の最小呼出 (CI スキップ、ローカル / 手動)
...
|---|---|---|
| §13.1 「`ANTHROPIC_API_KEY` set 状態で `autokit run` が exit 1」「runner 子プロセスの env に `ANTHROPIC_API_KEY` ... 継承されない」 | env 2 系統 allowlist / API key 継承防止 | `core/env-allowlist` (`buildGhEnv()`/`buildRunnerEnv()`)、`claude-runner/auth.ts` spawn 順序 (S2 D1) |
...
| §13.4 「audit kind が `failure.code` と 1:1 対応」「操作系 audit kind ... が info で必ず記録」 | audit kind 1:1 + 操作系 14 kind | `core/logger` table-driven |
...
| §13.5 「runtime_phase / agent_phase の用語分離」「AgentRunOutput.status と prompt_contract YAML status が完全同値」「7 prompt_contract ... の `data` が §9.3 の厳密 schema」「prompt_contract id ... が step 名と完全一致」 | runtime_phase/agent_phase 分離 + AgentRunStatus 同値 + strict schema + prompt_contract 1:1 + skill 配置 | `core/doctor`、`claude-runner`/`codex-runner` schema fixtures、§5.3 fixture spec |
...
| review-only: SPEC §9.1.1 B 採用基準 | primary Codex SDK 3 phase (plan_verify / implement / fix) parse>=95% / schema validation>=95% / resume 100% (N=20) | S0 Exit B、同上 |
| review-only: SPEC §9.1.1 C 採用基準 | experimental Agent SDK parse>=95% / resume 100% (N=50, resume 10) | S0 Exit C、未達なら `sdk-experimental.ts` を S2 D2 から削除 |
...
| Claude Agent SDK の auth / settingSources の subscription 経路 | 高 | S0 で確認、API key fallback は持たない方針なので不可なら Agent SDK は採用見送り |
| Codex SDK sandbox / approval policy で push / gh が動かない | 中 | git/gh は core 専有。implement で network 必要時は paused |
...
| API key set 時の意図せぬ課金 | 高 | doctor FAIL + `core/env-allowlist` 2 系統で API key 子プロセス継承遮断 |
...
Claude Agent SDK / Codex SDK の TS パッケージ名は S0 で Context7 / 公式 docs 確認後に確定。`claude` / `codex` CLI は外部依存 (PATH 経由)。
...
- **重要原則 2:** API key を新規参照しないこと (subscription only、`core/env-allowlist` の `buildRunnerEnv()` で子プロセス継承遮断)
...
4. **S0 着手 (Runner / SDK Contract Spike) → exit 達成後に S1 へ**
