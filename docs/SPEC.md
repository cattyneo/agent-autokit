...
│   ├── .lock/              # cross-process run lock (gitignore, mode 0700)
│   ├── plans/              # プラン Markdown
...
serve:
  lock:
    host_redact: false       # true なら .autokit/.lock holder host を sha256 16 hex に置換
  sse:
    max_connections: 8
    heartbeat_ms: 15000
...
| `preset_path_traversal` | (preset apply abort) | `autokit preset apply` の preset entry / 展開先 / `.agents` parent chain で絶対 path、`..`、NUL、symlink、repo 外 realpath を検出 |
| `preset_blacklist_hit` | (preset apply abort) | `autokit preset apply` / `preset show` の path blacklist、content signature、または protected array 違反を検出 |
...
Phase 2A 以降、常駐 serve / CLI 直叩き / 将来 Dashboard の cross-process 排他には別 path `.autokit/.lock/` directory lock を使う。`.autokit/.lock/` は `fs.mkdir` による atomic directory creation で取得し、directory mode は 0700、holder metadata `.autokit/.lock/holder.json` は mode 0600。holder は `holder_token` / `pid` / `started_at_lstart` / `host` / `acquired_at` を持ち、release は `holder_token` 一致時のみ `holder.json` unlink + `rmdir(".autokit/.lock")` を行う。取得失敗は `serve_lock_busy` の fast-path とし、`failure.code` は発火しない。
...
3. **backup:** 上書き対象を `.autokit/.backup/<timestamp>/` (mode 0700) に退避
   ...
   - **retention:** `config.init.backup.retention_days` (default: `30`) を超えた既存 backup residue は init preflight で削除する。削除に失敗した場合は fail-closed し、中間状態のまま init を続行しない。
...
| `preset_apply_started` | `autokit preset apply <name>` が lock 取得後、backup / staging 展開を開始 |
| `preset_apply_finished` | final doctor 相当検証が緑で apply 成功、または rename 後 doctor failure から rollback 完了し `.agents` が apply 前 manifest と一致 |
| `preset_apply_rollback_started` | rename 後 doctor failure を検出し、backup 復元を開始 |
| `preset_apply_rollback_finished` | backup 復元が完了し、apply 前 manifest と一致 |
| `preset_apply_rollback_failed` | backup 復元失敗 / timeout / manifest 不一致で壊れた `.agents` が残り得る |
...
- `autokit init`: `<repo>/.autokit/.backup/<timestamp>/` (mode 0700)
- `autokit preset apply`: `${XDG_STATE_HOME:-~/.local/state}/autokit/backup/<repo-id>/<timestamp>/` (mode 0700)
  - `<repo-id>` は `sha256(realpath(repoRoot)).slice(0, 16)` の 16 hex chars。basename が同じ別 repo の backup が交差しないように realpath を入力にする。
  - `${XDG_STATE_HOME:-~/.local/state}/autokit/backup/<repo-id>/<timestamp>/` までの autokit 管理 parent chain は mode 0700 で作成する。
- `config.init.backup.retention_days` (default: `30`) を init / preset apply backup に共通適用する。期限切れ backup の削除に失敗した場合は fail-closed し、backup / staging / write を開始しない。
- `autokit preset apply` は backup / staging / write 前に `.agents` parent chain と preset 展開先を chained-openat 相当の親 chain realpath 検査へ通し、symlink / repo 外 realpath / NUL / `..` を `failure.code=preset_path_traversal` で fail-closed する。
...
- [ ] **操作系 audit kind (§10.2.2.1 の 31 種: resume / resumed / lock_seized / init_rollback / init_rollback_failed / retry_resumed / runner_idle / audit_hmac_key_rotated / queue_corruption_recovered / sanitize_pass_hmac / auto_merge_disabled / auto_merge_reserved / branch_deleted / retry_pr_closed / effort_downgrade / phase_self_correct / phase_started / phase_completed / review_finding_seen / fix_started / fix_finished / review_started / phase_override_started / phase_override_ended / serve_lock_busy / sse_write_failed / preset_apply_started / preset_apply_finished / preset_apply_rollback_started / preset_apply_rollback_finished / preset_apply_rollback_failed) が info または指定 level で必ず記録**
