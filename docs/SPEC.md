...
`autokit` は GitHub Issue を自律実装する CLI オーケストレーター。Claude (`claude -p` runner primary) と Codex SDK を呼び出し、プラン作成 → 実装 → レビュー → マージまで順次自動実行する。git/gh/PR/merge 操作は core が単独所有し、agent はコード編集と検証のみ担当する。
...
| runner | provider 呼出層。`claude -p` runner / Codex SDK runner |
...
- 環境変数 `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` は **unset 必須**
  - subscription 認証優先のため。set されていれば `run` / `doctor` は **FAIL → exit 1**
...
│   ├── codex-runner/              # Codex SDK wrapper
│   └── tui/                       # Ink components
├── docs/
│   ├── SPEC.md
│   └── PLAN.md
...
2. **token-like pattern 置換:** `<REDACTED>` 化
   ...
   - `(refreshToken|oauthAccessToken|access_token|refresh_token)["']?\s*[:=]\s*["'][^"']+["']` (Claude/Codex subscription credentials JSON)
   - `ssh-rsa\s+[A-Za-z0-9+/=]+`
...
| `provider_sessions.<phase>.{claude_session_id,codex_thread_id}` | task → provider_sessions 配下 | agent_phase 7種の session resume |
...
1. **agent_phase (`plan` / `plan_verify` / `plan_fix` / `implement` / `review` / `supervise` / `fix`):**
   ...
   2. `git.checkpoints.<該当 phase>.before_sha` のみ存在 + `provider_sessions.<該当 phase>` に session_id/thread_id あり → runner で resume API 呼出
...
| **env unset (process.env)** | autokit process 自身の `process.env` で `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` 未設定 | **FAIL** |
| **login-shell env probe** | `zsh -ic 'env'` および `bash -lc 'env'` (user の login shell に応じて) を spawn し、出力中に `ANTHROPIC_API_KEY=` / `OPENAI_API_KEY=` 行が含まれない (= `~/.zshrc` / `~/.bash_profile` / `.envrc` (direnv) で API key を export していない) | **FAIL** (subscription 前提崩壊、子 process spawn 時 shell rc source で key 再取得経路を block) |
| **cwd `.env` 検査** | autokit run 起動 cwd の `.env` / `.env.local` / `.env.<NODE_ENV>` に `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` 行が含まれない | **FAIL** (CLI 自身が cwd `.env` を読込む経路を block) |
| **`~/.claude/settings.json` env field** | `~/.claude/settings.json` が `env` block を含み、その中に `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` キーが存在しない | **FAIL** (claude CLI 経由の env 注入経路を block) |
...
TUI (Ink prompt) → ユーザー入力
   ↓
回答を runner に resume (session_id / thread_id 経由)
```
...
- **Codex: Codex SDK (TS) primary**。`runStreamed` + `resumeThread`
- **API key fallback は MVP で持たない。** subscription auth のみ
- skill / agent は SDK option ではなく **filesystem + prompt 明示** で読み込ませる
...
##### B. primary runner: Codex SDK (Codex phase 全 3 種: plan_verify / implement / fix、§2.2 役割分担表と 1:1)

`autokit` v0.1.0 が依拠する **唯一の Codex runner**。MVP に含めるため必達。
...
- `runStreamed` で worktree 内編集 + sandbox 制御:
...
- subscription 認証 (`codex login`) で動作 (`OPENAI_API_KEY` unset でも実行可)
- `runStreamed` / `resumeThread` / config override / sandbox / approval policy の exact TypeScript API と config key が pinned version の型・公式 docs・実機で一致
...
prompt YAML status と AgentRunStatus は完全同値のため、runner はマッピング変換せずそのまま AgentRunOutput.status に転写する。`rate_limited` のみ runner が provider error code / HTTP status / response body から判定して付与する。

`status=completed` のとき `data` は phase 固有 schema に合致必須。`status=need_input` のとき `question.text` / `question.default` が必須で、`data` は任意だが存在する場合は該当 phase schema に合致する。`status=paused` / `failed` のとき `data` は `{ reason: string, recoverable?: boolean }` のみ許可する。未知 field は拒否する。すべての string は sanitize 後 16KB 以下、array は明記上限以下、enum は下表の値のみ。runner は YAML parse 後に schema validation を行い、違反時は E34 (`failed` + `failure.code=prompt_contract_violation`) として扱う。`structured` は validation 済みの `data` のみを格納し、raw YAML は保存しない。
...
`ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / 任意のユーザー env / `.env` ファイル由来 env / `AUTOKIT_*` (autokit internal は子 process には不要) は **全系統で渡さない**。
...
- `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` は子プロセスに継承しない (allowlist 方式で env scrub)
...
- [ ] `autokit doctor` が必須項目を全て検査
  - [ ] env unset (`ANTHROPIC_API_KEY` / `OPENAI_API_KEY`) で **FAIL** 表示
...
- [ ] **runner 子プロセスの env に `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` が継承されない**
...
- [ ] **両系統とも `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / 任意ユーザー env / `.env` 由来 env / `AUTOKIT_*` を継承しない**
...
- env: `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` unset、claude / codex / gh 認証済
...
- Codex SDK / agents: https://developers.openai.com/codex/
