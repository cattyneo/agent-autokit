...
Codex SDK primary 前提の SPEC / PLAN / Issue train を、API費用を発生させない `codex exec` primary 前提へ修正し、実装再開可能な状態に戻す。
...
- `codex exec --json` / `--output-schema` / `codex exec resume <session_id>` を Codex runner の基本形として記述する。
...
- `OPENAI_API_KEY` / `CODEX_API_KEY` set 時の検出動作を確認する。
...
- `codex exec` の one-shot smoke は subscription-auth / API key unset を確認してから実行する。
