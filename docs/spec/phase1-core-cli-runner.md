...
旧 yaml ( `{ claude_session_id }` 単独 / `{ codex_session_id }` 単独) は zod default 化で吸収 (`last_provider` を未設定許容)。
...
- override 時も state-machine 経由 (新 `TransitionEvent: phase_override_started` / `phase_override_ended` を追加、`cross-cutting.md` §3)、runner 直叩き不可
...
`doctor` + run 開始時に以下を検出:

- 「override 後 effective permission が phase 既定より緩い」→ fail-closed
- 「`(phase, provider)` が capability table 不許可」→ fail-closed
...
- state-machine に新 `TransitionEvent: prompt_contract_self_correct` を追加 (`cross-cutting.md` §3)。workflows 内 try/catch で隠蔽せず、必ず state-machine 経由
- 操作系 audit kind `phase_self_correct` を発火 (`cross-cutting.md` §2.1)
...
- 新: 「prompt_contract 違反 + `runtime.phase_self_correct_done=true` (= self-correction retry 1 回目で再度違反)」で `failed`。`phase_self_correct_done=false` 状態の 1 回目違反は新 `TransitionEvent: prompt_contract_self_correct` 経由で同 phase 内 retry へ遷移 (`cross-cutting.md` §3)
