...
| 主体 | 責務 |
|---|---|
| `packages/core/src/capability.ts` (新設) | 全 7 phase × 2 provider の **14 組すべてを許可組** として保持。`permission_profile` は provider ではなく phase 表 (§1.5) で固定し、derive 関数はその hard cap 内の権限だけを返す |
...
- capability table 由来の `denied_tools` / `permission_profile` を **hard cap** とし、deprecated `allowed_tools` は hard cap 内の追加許可だけを受理する。read-only profile で `Edit` / `Write` / `Bash` を指定した config は warn ではなく fail-closed
...
- provider は 7 phase × 2 provider の全 14 組を許可する。不正な phase / provider 値、`ci_wait` / `merge` への provider 指定、または phase 固定の permission_profile を緩める設定は `doctor` と `run` 開始時に fail-closed
...
- Codex は現行 CLI contract に合わせ、runner 起動時に `-c model_reasoning_effort=<low|medium|high>` を渡す。`--reasoning-effort` flag は前提にしない
...
- `runner_timeout` optional 化と同じ PR で、core に default 解決済み timeout API (`resolveRunnerTimeout(config, phase, resolvedEffort?)` など) を置き、CLI / workflows の既存 consumer は **必ず number を返す API** を呼ぶ。中間 PR で `phaseTimeoutMs()` 等が `number | undefined` を返す状態は禁止
...
- `resolveRunnerTimeout(config, phase, resolvedEffort?)` (名称仮) を core に追加し、optional 化後も全 consumer が `number` を受け取るよう同 PR で更新
...
```ts
provider_sessions: {
  [phase: Phase]: { claude_session_id?: string | null; codex_session_id?: string | null; last_provider?: Provider | null } | null;
}
```
...
- `resume` は `runtime.phase_override` が有効なら override provider、なければ `provider_sessions.<phase>.last_provider`、それもなければ config/capability の effective provider を使う
- `plan --provider codex` / `implement --provider claude` のような旧 default と逆の provider 実行後も、crash / resume で同じ provider session に戻る fixture を必須にする
...
- write profile 用 PreToolUse hook 新設: `write_path_guard`
  ...
  - 以下への書込を fail-closed:
    ...
    - `*.pem`
    - `*.key`
- Bash 許可コマンド allowlist: `build` / `test` / `lint` / `format` 系。ただし provider Bash が package script を **直接** 実行することは禁止し、実行は autokit-owned guarded command runner 経由に限定する
- `git` / `gh` は guarded command runner 経由の read-only 参照を許可し、write / state-changing 操作を deny:
  - allow: `git status` / `git diff` / `git show` / `git log` / `git blame`、`gh issue view|list` / `gh pr view|list` / read-only `gh api --method GET`
  - deny: `git add|commit|checkout|switch|reset|rebase|merge|push|tag|branch -d|-D|worktree remove`、`gh pr create|merge|close|comment|review|edit`、`gh issue create|edit|close|comment`、非 GET `gh api`
  - 許可 git/gh も secret path guard と output redaction を必ず通す。`.env*` / `.codex/**` / `.claude/credentials*` / key files を pathspec / revision path (`HEAD:.env` 等) / API path に含む場合は fail-closed。出力は public redaction API (`packages/core/src/redaction.ts`) で sanitize してから runner に返す
  - 判定不能な `git` / `gh` 呼び出しは fail-closed
- package script 経由の迂回禁止: guarded command runner は cwd を worktree に固定し、HOME / XDG / git credential / gh config を隔離、secret path (`.env*` / `.codex/**` / `.claude/credentials*` / key files) は read/write とも deny、`.git/**` は write deny。PATH shim に加え、absolute `/usr/bin/git` や absolute `gh` が write 系を試みても `.git` write / forbidden path policy で fail-closed する。v0.2 は arbitrary network syscall (`node fetch` / `curl` / dependency lifecycle script 等) の OS レベル遮断を保証しない。`WebFetch` / `WebSearch` deny、git/gh write deny、credential/env 隔離で外部送信リスクを下げ、OS/process-level network sandbox は将来 hardening として別 Issue 化する
- write profile の HOME / XDG / git credential / gh config は runner 専用 scratch に隔離し、operator の `$HOME` / `~/.gitconfig` / `~/.config/gh` / credential helper へ到達させない。`GIT_CONFIG_GLOBAL=/dev/null`、`GIT_CONFIG_NOSYSTEM=1`、`GH_CONFIG_DIR=<runner-scratch>/gh` 相当の環境隔離を runner contract とする
...
- effort は `-c model_reasoning_effort=<value>` で渡す (現状 `buildCodexArgs` (`codex-runner/src/index.ts:152-178`) は `--model` / `--sandbox` まで、本 Phase で config override を追加)
...
### 6.2 fail-closed 検査 (permission_profile hard cap)

CLI override で許容されるのは `(provider, effort)` のみ。provider は全 14 組で許可されるが、`permission_profile` は phase 表 (§1.5) で固定し provider 切替では変えない。`doctor` + run 開始時に以下を検出して fail-closed:

- override 後の `permission_profile` が phase 固定値と一致しない
- `ci_wait` / `merge` など agent_phase 7 種以外に provider override が指定される
- deprecated `permissions.claude.allowed_tools` 等により hard cap を超える tool / sandbox / network が有効化される
...
- runner が prompt_contract parse error / `requireStructuredData` failure / prompt_contract mismatch を検出した場合、初回から fatal exception として workflow 外へ投げない。`prompt_contract_violation` を表す recoverable result (または typed recoverable error) として workflow の `runWithSelfCorrection` に渡す
- `runWithSelfCorrection` は retry 前に `runtime.phase_self_correct_done=true` 永続化 → `phase_self_correct` audit emission → checkpoint write の順序を保証する。crash がこの途中で起きても `resume` は二重 retry せず、永続化済み状態に従う
- `phase_self_correct_done=true` 状態で同じ recoverable violation を受け取った場合のみ state-machine E34 に渡し、`failure.code=prompt_contract_violation` で failed 化する
- runner process 起動失敗 / timeout / sandbox 起動不能など prompt_contract 以外の例外は self-correction 対象外の fatal error として既存経路で扱う
...
- `packages/core/src/redaction.ts` (新設) に public redaction API (`sanitizeLogString` / diff 用 redactor) を切り出し、`packages/core/src/index.ts` から export する。既存 logger / CLI `logs` / CLI `diff` / serve SSE は同一 API と同一 fixture を使い、redaction 挙動を分岐させない
- workflow state write 境界 (`tasks.yaml` 永続化前) も同じ public redaction API を必ず通す。runner summary / error / prompt_contract violation message / supervisor reject reason / review finding text など provider 由来または prompt 由来の free text を、`failure.message` / `failure_history` / `review_findings.*` / `reject_history.reason` に保存する前に sanitize する
- `autokit logs` 出力時に public `sanitizeLogString` で **再 sanitize** (二重 redact)
- rotated log を時系列結合してから sanitize

#### AC 追加

- [ ] `failure.message` / `failure_history[].message` / `review_findings[].title|body|file` / `review_findings.reject_reasons` / `reject_history[].reason` に dummy token (`ghp_*` / `sk-*` / `Bearer ...`) や `.env` 値が残らない fixture
- [ ] sanitize 後も operator が原因を追えるよう、redacted placeholder (`<REDACTED>`) と phase / failure.code は保持する
...
path-based 除去を通過した残り全 hunk の body に **public redaction API の `sanitizeLogString`** (SPEC §4.6.2.2 token-like pattern + `logging.redact_patterns`) を必須適用。`README.md` / `config/example.ts` 等の非ブラックリスト path に commit / staged された credential もマッチング行を `<REDACTED>` 化:
