...
MIG-004 以降の v0.1.0 Codex runner 方針は **Codex SDK primary ではなく `codex exec` primary**。Codex SDK `runStreamed` / `resumeThread` evidence は deferred / paid-risk-gated reference としてのみ扱い、AK-010 の adoption gate には使わない。`codex exec` live smoke の追加実行は operator が実行範囲を承認し、`ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `CODEX_API_KEY` unset を確認してから行う。
...
Help-confirmed pinned CLI option surface (2026-05-05, local help / auth / env only, no new live model call):
...
- The final user-facing structured output must come from final JSON / output schema validation (`--output-schema`, optionally `--output-last-message` / `-o` after evidence), not from YAML parsing.
...
| `codex exec --json` | Confirmed by help + existing smoke | Help lists `--json`; AK-001 smoke emitted JSONL with `thread_id` / `agent_message` |
| `--output-schema` | Confirmed by help + existing smoke | Help lists `--output-schema <FILE>`; AK-001 smoke used schema |
| `--output-last-message` / `-o` | Confirmed by help only | Needs AK-010 fixture / implementation validation before being required |
| `codex exec resume [SESSION_ID]` / `--last` | Confirmed by help only | Needs live resume smoke before AK-010 treats resume as adopted |
| sandbox flag | Confirmed by help only for CLI | `--sandbox` values are listed; live read-only/workspace-write CLI sandbox evidence still required |
| approval fail-closed | Confirmed by global help semantics only | `-a never` says execution failures are returned; AK-010 must test approval prompt fail-closed |
| live `codex exec` one-shot during MIG-004 | Not run | Requires operator approval and API key unset confirmation immediately before execution |

Current decision: `codex exec` is the v0.1.0 primary Codex runner path. PATH Codex CLI 0.128.0 is now the pinned local CLI for help / auth / non-live evidence, but AK-010 cannot rely on unexercised resume / output-file / sandbox / approval details until live or fixture evidence pins those exact contracts. Any unconfirmed required CLI feature stops AK-010 implementation.
