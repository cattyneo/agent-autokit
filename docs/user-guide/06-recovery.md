...
### 5. CI failure ループ枯渇（`ci_fix_exhausted` 系）

`ci.fix_max_rounds`（デフォルト 3）回試しても CI が直らない。
...
### 6. CI timeout（`ci.timeout_action`）

`ci.timeout_ms`（30 分）超過。デフォルトでは `paused` に落ちる。

**対処**: GitHub Actions が単に遅いだけ（runner 待ち等）なら、CI 完了を待ってから `autokit run` を再実行。常態的に長引くワークフローなら `config.yaml` で `ci.timeout_ms` を引き上げる。
...
`.autokit/.backup/` 系から tasks.yaml を復元し、対象 issue の存在を確認するのみ（state 復帰は手動）。復元後は `autokit list` で内容を確認。
