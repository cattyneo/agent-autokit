...
├─ claude-runner    # @cattyneo/autokit-claude-runner   Claude CLI ラッパ
├─ codex-runner     # @cattyneo/autokit-codex-runner    Codex CLI ラッパ
...
`autokit run` は **長寿命プロセスを持たない**。runner / gh / git I/O は子プロセス起動で行う。`autokit serve` は例外で、local HTTP API / SSE の長寿命プロセスとして動き、SIGINT / SIGTERM で token file と server を cleanup する。
...
flowchart LR
  ...
  AK -. spawn .-> claude["claude CLI<br/>(permission from phase)"]
  AK -. spawn .-> codex["codex CLI<br/>(permission from phase)"]
  AK -. spawn .-> gh["gh CLI"]
  AK -. spawn .-> git["git"]
