...
```mermaid
flowchart TB
  subgraph readonly["readonly phases"]
    rP["plan / plan_verify / plan_fix"]
    rM["permission:<br/>readonly_repo"]
  end

  subgraph review["review phases"]
    vP["review / supervise"]
    vM["permission:<br/>readonly_worktree"]
  end

  subgraph write["write phases"]
    wP["implement / fix"]
    wM["permission:<br/>write_worktree"]
  end
  ...
  review -. read-only .-> R
  write -. "read/write only (worktree path)" .-> R
```

役割分離の不変条件:

- capability table は 7 agent phase × 2 provider の全組を許可するが、permission profile は provider ではなく phase から導出する
- **review / supervise は provider が変わっても readonly_worktree**（中立性 + 副作用なし）
