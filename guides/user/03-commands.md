...
JSON は `{ issue, state, runtime_phase, review_round, ci_fix_round, resolved_model, failure }` と runtime 由来の要約を返す。詳細な `resolved_effort` / `phase_override` / `provider_sessions` は `autokit list --json` の task entry で確認する。
