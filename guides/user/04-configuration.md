...
| `default_ms` | unset | 未指定時は phase default または resolved effort timeout へフォールバック |
...
phase 固有 timeout が未指定なら、`resolveRunnerTimeout(config, phase, resolvedEffort)` が effort 由来 timeout、`default_ms`、phase default の順で必ず number を返す。
