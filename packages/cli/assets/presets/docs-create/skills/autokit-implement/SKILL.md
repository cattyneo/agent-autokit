...
## Rules
- Read the issue, plan, SSOT, existing docs, affected commands, and current diff before editing.
- Keep docs concise, internally consistent, and grounded in verified behavior.
- Use RED / GREEN / REFACTOR when docs behavior or command examples are covered by checks.
...
- Delegate docs-affecting updates to `doc-updater` only when the issue calls for split documentation work.
